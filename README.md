...
3. Install pre-requisites ( sudo apt-get install docker docker-compose git -y && git clone https://github.com/cedrickoch/BitcoinNode.git && cd BitcoinNode && ./install.sh )
...
5. Change to the installation directory ( e.g. cd /home/bitcoin )
6. Clone the repository ( git clone https://github.com/cedrickoch/BitcoinNode.git )
7. Change to the BitcoinNode folder ( cd BitcoinNode )
8. Download the bitcoin data snapshot ( curl https://www.lopp.net/download/utxo-snapshot-height-840000.dat?ref=blog.lopp.net --output utxo-snapshot-height-840000.dat )
9. Import the bitcoin snapshot. It will take a while ( docker-compose -f docker-compose-import.yml up )
10. Start the bitcoin node ( docker-compose up -d )
